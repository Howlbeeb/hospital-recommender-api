...
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY", "YOUR_API_KEY")  # Set in Render environment variables
gmaps = googlemaps.Client(key=GOOGLE_API_KEY)

# Default coordinates for Lagos center
...
def get_driving_route(user_coords, hospital_coords, hospital_name):
    ...
    try:
        ...
        print(f"Error fetching route to {hospital_name}: {e}")
        return None, None, None, None
...
def setup_fuzzy_system():
    ...
    rules = [
        ...
        ctrl.Rule(
            service_match["high"] & proximity["near"] & quality["high"] & user_rating["high"] &
            (cost["low"] | cost["medium"]) & (user_cost_pref["low"] | user_cost_pref["medium"]) &
            user_quality_pref["high"],
            recommendation["high"]
        ),
        ctrl.Rule(
            service_match["high"] & proximity["very_near"] & quality["medium"] & user_rating["medium"] &
            cost["low"] & user_cost_pref["low"],
            recommendation["high"]
        ),
        ctrl.Rule(
            service_match["high"] & proximity["medium"] & quality["high"] & user_rating["high"] &
            (cost["medium"] | cost["high"]) & (user_cost_pref["medium"] | user_cost_pref["high"]),
            recommendation["high"]
        ),
        ctrl.Rule(
            service_match["high"] & proximity["near"] & quality["medium"] & user_rating["medium"] &
            cost["medium"] & user_cost_pref["low"],
            recommendation["medium"]
        ),
        ctrl.Rule(
